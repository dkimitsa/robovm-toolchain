...
for T in ${TARGETS[@]}; do
  ...
  bash -c "cd '$BASE/target.${SUFFIX}/build/$T'; cmake -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DOS=$OS -DARCH=$ARCH $CMAKE_PARAMS $CMAKE_INSTALL_DIR '$BASE'; $MAKE robovm-libmd"
  R=$?
  if [[ $R != 0 ]]; then
    echo "$T build failed"
    exit $R
